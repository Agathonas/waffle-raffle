...
mod Raffle {
    ...
    impl RaffleImpl {
        ...
        fn enter_raffle(ref self: ContractState, raffle_id: u256, entries: u256) {
            ...
            let user_entry_count = self.user_entries.read((caller, raffle_id)).unwrap_or(Zeroable::zero());
            self.user_entries.write((caller, raffle_id), user_entry_count + entries);
            self.raffles.write(
                raffle_id,
                Raffle {
                    item: raffle.item,
    ...
    impl InternalImpl {
        ...
        fn get_winner_by_index(self: @ContractState, raffle_id: u256, index: u256) -> ContractAddress {
            ...
            for user in users {
                let entry_count = self.user_entries.read((user, raffle_id)).unwrap_or(Zeroable::zero());
